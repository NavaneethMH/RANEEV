...
import type { EmergencyMapController, EmergencyMapData, RouteMetrics } from "@/lib/maps/contracts";

type EmergencyMapProps = { data: EmergencyMapData; className?: string; onRouteMetrics?: (metrics: RouteMetrics | null) => void; label?: string };

export function EmergencyMap({ data, className = "", onRouteMetrics, label = "Authorized emergency map" }: EmergencyMapProps) {
  ...
  if (mapFailed) return <div className={`relative flex min-h-[26rem] flex-col justify-between overflow-hidden rounded-xl border border-[#e8c9c2] bg-[#fff7f4] p-6 ${className}`}><div><div className="flex items-center gap-2 text-[#a12f24]"><TriangleAlert className="h-5 w-5" /><p className="text-sm font-extrabold">Map temporarily unavailable</p></div><p className="mt-3 max-w-md text-sm font-medium leading-6 text-[#52626c]">RANEEV could not load the live map provider. The confirmed incident location and responder status remain available below while the provider is retried.</p></div><div className="border-t border-[#ecd5cf] pt-4"><p className="rn-eyebrow">Confirmed incident location</p><p className="mt-2 rn-mono text-xs font-bold text-[#43535e]">{data.incident.position.lat.toFixed(5)}°, {data.incident.position.lng.toFixed(5)}°</p>{data.responder && <p className="mt-3 text-sm font-extrabold text-[#247352]">Responder position remains server-confirmed.</p>}</div><button onClick={() => { setMapFailed(false); setReady(false); }} className="rn-focus mt-5 min-h-11 self-start rounded-md border border-[#0e4e78] bg-white px-4 text-xs font-extrabold text-[#0e4e78]">Retry map</button></div>;

  return <div className={`relative overflow-hidden rounded-xl border border-[#bfc9cb] bg-[#dfe7e5] ${className}`}>
    <MapView className="h-full min-h-[26rem] w-full" initialCenter={data.incident.position} initialZoom={14} onMapReady={async map => { try { controller.current = await mountGoogleEmergencyMap(map, mapDataRef.current, onRouteMetrics); setReady(true); } catch { setMapFailed(true); } }} onMapError={() => setMapFailed(true)} />
