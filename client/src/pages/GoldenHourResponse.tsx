...
import { AlertTriangle, Building2, CheckCircle2, Clock3, Navigation, PhoneCall, Route, ShieldAlert, TimerReset, UserRoundCheck } from "lucide-react";
...
function TimeSinceIncident({ createdAt }: { createdAt: Date | string }) {
  ...
  return <div className="border-l-4 border-[#d83232] bg-[#fff7f4] px-4 py-3 shadow-[var(--rn-shadow-sm)]"><div className="flex items-center gap-2 text-[#a52828]"><Clock3 className="h-4 w-4" /><span className="text-[0.65rem] font-extrabold uppercase tracking-[0.13em]">Time since incident</span></div><p role="timer" aria-live="polite" className="mt-1 rn-mono text-3xl font-extrabold tracking-[-0.06em] text-[#17212b]">{formatElapsed(now - startedAt)}</p><p className="mt-1 text-xs font-semibold text-[#60707c]">Started {new Date(createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</p></div>;
}

function incidentTone(status: string) {
  if (status === "resolved") return "safe" as const;
...
export function GoldenHourResponse({ publicId }: { publicId?: string }) {
  ...
  if (!publicId) return <><PageHeading eyebrow="Golden Hour Response" title="Open the time-critical incident sequence." description="GHR is an operational layer on the existing emergency record—not a parallel dashboard. Select an active incident to coordinate the next safest response." action={<StatusBadge tone="critical" label={`${active.length} active`} />} /><div className="grid gap-5 xl:grid-cols-[1fr_0.42fr]"><SurfaceCard tone="critical" className="p-6"><div className="flex items-center gap-3"><TimerReset className="h-6 w-6 text-[#d83232]" /><div><p className="text-lg font-extrabold">Active incident queue</p><p className="mt-1 text-xs font-semibold text-[#60707c]">Prioritize elapsed time, response state, and appropriate facility coordination.</p></div></div><div className="mt-5 divide-y divide-[#edf0ee]">{activeIncidents.isLoading ? <p className="py-8 text-sm font-semibold text-[#60707c]">Loading protected incident queue…</p> : active.length ? active.map(incident => <Link key={incident.id} href={`/coordinator/ghr/${incident.publicId}`} className="rn-focus flex min-h-20 items-center justify-between gap-4 py-4 first:pt-0 last:pb-0"><div><p className="text-sm font-extrabold text-[#17212b]">{emergencyLabels[incident.emergencyType] ?? incident.emergencyType}</p><p className="mt-1 text-xs font-semibold text-[#60707c]">{incident.publicId} · {incident.locationLabel}</p></div><StatusBadge tone={incidentTone(incident.status)} label={incident.status.replace("_", " ")} /></Link>) : <p className="rounded-lg border border-dashed border-[#cad4d5] p-5 text-sm font-semibold text-[#60707c]">No active shared incidents need Golden Hour coordination.</p>}</div></SurfaceCard><div className="space-y-4"><OperationalAlert tone="warning" title="Operational priority only">Severity labels coordinate response effort; they are not a medical diagnosis. Contact local professional emergency services whenever available.</OperationalAlert><StatePreview compact /></div></div></>;
...
function GoldenHourIncidentWorkspace({ publicId }: { publicId: string }) {
  ...
  const refresh = useCallback(() => {
    utils.ghr.overview.invalidate({ publicId });
    utils.incidents.timeline.invalidate({ publicId });
    utils.coordinator.activeIncidents.invalidate();
  }, [publicId, utils]);
